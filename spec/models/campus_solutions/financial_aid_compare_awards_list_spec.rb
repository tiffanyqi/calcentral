describe CampusSolutions::FinancialAidCompareAwardsList do
  ...
  context 'real proxy', testext: true do
    let(:proxy) { CampusSolutions::FinancialAidCompareAwardsList.new(user_id: user_id, fake: false, aid_year: 2016) }
    it_should_behave_like 'a proxy that gets data'

    context 'an invalid request' do
      let(:proxy) { CampusSolutions::FinancialAidCompareAwardsList.new(user_id: user_id, fake: false, aid_year: 0) }
      subject { proxy.get }
