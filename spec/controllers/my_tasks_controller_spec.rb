  ...
  context 'delegated access' do
    ...
    context 'financial access' do
      ...
      it 'allows access only to Financial Aid tasks' do
        ...
        cs_finaid_tasks = json_response['tasks'].select {|t| (t['emitter'] == 'Campus Solutions') && t['cs']['isFinaid']}
        expect(cs_finaid_tasks).to be_present
        expect(json_response['tasks']).to eq cs_finaid_tasks
      end
    end
  end
